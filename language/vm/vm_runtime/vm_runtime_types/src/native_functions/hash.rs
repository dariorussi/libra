...
pub fn native_keccak_256(mut arguments: VecDeque<Value>) -> NativeReturnStatus {
    if arguments.len() != 1 {
    ...
    let return_values = vec![Value::byte_array(ByteArray::new(hash.to_vec()))];
...
pub fn native_ripemd_160(mut arguments: VecDeque<Value>) -> NativeReturnStatus {
    if arguments.len() != 1 {
        return NativeReturnStatus::InvalidArguments;
    }
    let hash_arg = pop_arg!(arguments, ByteArray);
    ...
    let return_values = vec![Value::byte_array(ByteArray::new(hash_ref.to_vec()))];
    NativeReturnStatus::Success {
        cost,
        return_values,
    }
