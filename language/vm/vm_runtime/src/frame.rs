...
use vm::{
    errors::{Location, VMInvariantViolation},
    ...
    F: FunctionReference<'txn>,
{
    ...
    pub fn copy_loc(&self, idx: LocalIndex) -> Result<Value, VMInvariantViolation> {
        self.locals.copy_loc(idx as usize)
    }

    pub fn move_loc(&mut self, idx: LocalIndex) -> Result<Value, VMInvariantViolation> {
        self.locals.move_loc(idx as usize)
    }

    pub fn store_loc(&mut self, idx: LocalIndex, value: Value) -> Result<(), VMInvariantViolation> {
        self.locals.store_loc(idx as usize, value)
    }

    pub fn borrow_loc(&mut self, idx: LocalIndex) -> Result<Value, VMInvariantViolation> {
